...
In computer science, a heuristic is a technique that allows you to solve a problem faster than you would by using a classic method. Or, when a classic method cannot return an *exact* answer, a heuristic technique would return an *approximate* answer. These techniques involve tradeoffs between speed and precision. Another way to think about it is that a heuristic produces a *“good enough”* solution when an *exact* solution would either take too long or is not possible to compute.
