...
**_Inheritance relationships_** can be defined between classes in situations where more specific child classes extend the functionality of more general parent classes. These hierarchies help us write well-organized code and can allow for code re-use.

##### Overview

### Inheritance

When you’re attempting to solve a programming problem, you will often find an existing class that does _almost_ what you need. If you changed the class, you could end up breaking something that used to work.
...
Instead of changing the old class or cutting and pasting to create a similar class, you can use _inheritance_ to solve your problem. When you use inheritance to create your new class, you can use all the code from the old class (copying none of it) and you can also add the additional functionality you need.

In the new class, we only need to define what we add or change from the old class. We call the original class either the _parent, superclass,_ or _base class_. We call the new class the _child, subclass_, or _derived class_.
...
Now that you have explored inheritance relationships, we will look at another category of class relationships - **_association_**.

##### Overview

Composition in programming is when we collect several objects together to create a new one. Composition is usually the right choice when one object is part of another object. It’s often said that if inheritance establishes an **_“is-a”_** relationship, then composition establishes a **_“has-a”_** relationship.

To illustrate, imagine you’ve created a `Duck` class. A duck is made up of many parts, or a duck **_has_** many parts. So, a `Tail` class would not inherit from the `Duck` class, rather a `Duck` would be composed of the `Tail` class (and many others like `Bill`, `Eye`, `Foot`, etc.).
...
Notice how none of the classes have an inheritance relationship with the others. However, the `Duck` class expects to receive instances of the `Bill` and `Tail` classes when it is initialized. You can tell this is a composition relationship because you can accurately describe the program by stating that the `Duck` **_has a_** `Bill` and `Tail`.

### Aggregation

Aggregation is almost exactly like composition. The difference is that aggregate objects can exist independently. It would be impossible for a bill to be associated with a different duck. But what if there was a `Collar` or `Leash` class? A `Duck` _could_ have a collar or leash (although it’d be strange I admit), and, unlike a `Bill` or `Tail`, the collar or leash could exist independently from the `Duck`.
...
1.  Think of your own example of objects that represent a **_has-a_** relationship. Define a class for each of the objects in such a way that accurately represents the composition relationship.
