...
- implement a hash table that uses a linked list for chained collision resolution and describe its performance
...
#### Learn to implement a hash table that uses a linked list for chained collision resolution and describe its performance

##### Overview

### Collisions

Up to now, we’ve been operating under the assumption that hash functions always map different inputs to different indexes. However, unless you knew all the possible data ahead of time, it would be impossible to write a hash function that was perfect in this regard. In actuality, hash functions do occasionally return the same integer, given different inputs.
...
![Collision Chaining Example](https://tk-assets.lambdaschool.com/f0d56e46-1bdb-4a2f-9121-0f235db6dd4b_S5-M2-O1-Collision-Chaining-Example.gif)
...
1.  Go through the U and P steps of UPER to come up with pseudocode for a hash table implementation that uses a linked list for chained collision resolution.
...
![Load Factor Definition](https://tk-assets.lambdaschool.com/59d00218-52e2-4f3d-9680-2b2d8baad3ae_S5-M3-O1LoadFactor.001.jpeg)
...
1.  Is it reasonable to have a higher maximum load factor on a hash table with chaining for collision resolution than you would on a hash table with linear probing?
2.  Why is it essential for a HashTable class to allow the user to determine the initial size? For example, if you had 50,000 items that you wanted to populate a hash table with immediately, why would being able to decide on the initial size be essential?
...
- **[Hash Tables II for CSPT12 w/Tim Roy](https://youtu.be/HbHiadquSK8)**

  How to implement a hash table that uses a linked list for chained collision resolution.

- [All previous recordings](/archive/CS/module/recHzCwboKBLBB0Re)
