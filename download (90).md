...
#### Learn to use function declarations, expressions, and arrow functions and describe their differences

Functions are everywhere in JavaScript. Being able to decipher between function syntax, concepts, and usage is vital to your success as a developer. As you continue to push deeper into JavaScript more concepts will unlock as you gain knowledge about functions.
...
This is how we capture data into memory in JavaScript. First, we declare a variable by calling `var` and deciding what that variable should be named, in this case: `name`. Then, we assign the variable (`name`) to a specific piece of data/information, in this case, the string ‘Batman’. For up to 5 standardized (over a decade of use with JavaScript), this was sufficient, one keyword `var` always got the job done.
...
What this means is that you can completely override the definition of the variable, `name`. This would be a problem in large applications with many ‘global’ (we’ll talk about global and local later) variables. In these circumstances, you can easily override information that may be completely important and vital to your application if you’re not super careful.

As a result of this ability to override and replace variables almost at will, [ES6’s let keyword](http://es6-features.org/#BlockScopedVariables) gives us the ability to not only combat this problem but, to solve problems we didn’t even know we had.

One of the best abilities we gain from `let` (AND `const`) is that variables cannot be overridden without throwing an error code. `let` allows us to solve this problem.

Let’s reference the example from above:
...
Let’s practice the concepts we just learned together. Use this link to follow along: [Variables 101](https://codepen.io/lambdaschool/pen/VBEWpP).
...
Objects are used all over in JavaScript. In fact, without the object, we don’t have the JavaScript language. Understanding how everything we do in JavaScript is built on objects is not only crucial to your understanding of the language, you won’t be able to go very far without knowing about them.

##### Overview

### Object Literals

Let’s learn how to create an `object literal` and assign it properties/values by use of `dot notation` and `bracket` notation. Objects can give programs access to their properties.
...
If we want to change “Fred’s” first name, all we’d have to do (just like a variable) is reference what we want to change and set it `=` to something.
...
[To review everything you learned above, click here for CodePen examples](https://codepen.io/lambdaschool/pen/rqYQmd?editors=0012).
...
Arrays are just a special type of object. The thing that makes arrays special is their ability to store ordered values. We gain a lot of abilities from using an array methods on our code. Let’s take a look at what an array is and how it works.

##### Overview

### Arrays

Let’s Learn about the fundamental usage of arrays in JavaScript, and how to define an array and assign it to a value.
...
Create an array of your favorite things. Use methods like `.push()` `.unshift()` `.splice()` to play around with your list.
