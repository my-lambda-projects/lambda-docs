...
#### Learn to make PUT requests to an external API using axios

When data needs to be `UPDATED` somewhere we use a PUT `HTTP resource` method to do so.
...
Let’s build in the AJAX call for the same app. You can start with [this codesandbox](https://codesandbox.io/s/k353v1z79r) tutorial and then follow along with the video.
