...
Inside this class, we will make an _inner class_ (nested class) called a `Meta` to tell it what parts of the model we want to access:
...
So far we’ve just exposed _all_ the data from a model to the entire world. But commonly it’s desirable to restrict access to data on a per-user basis. We’ll set that up with some additional Python code.
...
Since REST clients can’t use our login UI to set which user they are, they have to use an alternate method of authentication. In this portion of the code, we’ll use _token authentication_ to allow the user to log in via a new endpoint, and use that to control their access.
