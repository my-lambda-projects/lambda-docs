Please enable JavaScript to view this page.

You must be logged in to view this page.

You must be a Lambda School student to view this page.

<a href="#content" id="skippy" class="sr-only sr-only-focusable"></a>

<span class="skiplink-text">Skip to main content</span>

<a href="/" class="navbar-brand"><img src="data:image/svg+xml;base64," /></a>

![](data:image/svg+xml;base64,)

#### Computer Science Legacy

<a href="/cs/sprint/recd4D4w3QrigPqUF" class="bd-toc-link">1.  Intro to Python and OOP</a>

- [Introduction to Python I](/cs/module/recay2erzDlYUPSeO/)
- [Introduction to Python II](/cs/module/recwpe3Y9TVWrGT8L/)
- [Introduction to Python III](/cs/module/reca7NYptklr7F403/)
- [Introduction to Python IV](/cs/module/recc3eWphKVYd0oHT/)

<a href="/cs/sprint/recR4gHcvD21ziR9a" class="bd-toc-link">2.  Data Structures</a>

- [Data Structures I](/cs/module/rec3MaMAY78iDm7ax/)
- [Data Structures II](/cs/module/recMcvOrFw5BWUku3/)
- [Data Structures III](/cs/module/recx53S3pYfDfvFDm/)
- [Data Structures IV](/cs/module/recHdwPne4Xt3A7lk/)

<a href="/cs/sprint/recd9grrKlURJ453N" class="bd-toc-link">3.  Algorithms</a>

- [Iterative Sorting](/cs/module/reck76SPX26beGSqE/)
- [Recursive Sorting](/cs/module/reccRh9h6ccXghfA4/)
- [A First-Pass Solution](/cs/module/recrCuZQMVI6LvxhD/)
- [Writing Better Solutions](/cs/module/recsvJCzPlM2X63ZX/)

<a href="/cs/sprint/recvDjRQEq49uoWsU" class="bd-toc-link">4.  CS Unit 1 Build</a>

<a href="/cs/sprint/recAr3gdL8U57eho1" class="bd-toc-link">5.  Hash Tables</a>

- [Hash Tables I](/cs/module/recSwIvbSV630gdVk/)
- [Hash Tables II](/cs/module/recHzCwboKBLBB0Re/)
- [Hash Tables III & IV](/cs/module/recsEDFseukQWg92c/)

<a href="/cs/sprint/rec7U9K7OCL5ihj0t" class="bd-toc-link">6.  Graphs</a>

- [Graphs I](/cs/module/recBMbHtb8AOXq3UL/)
- [Graphs II](/cs/module/recZL2m6Gx7B4dU3G/)
- [Graphs III](/cs/module/reck4RVWsg82eiYPZ/)
- [Graphs IV](/cs/module/recoGWlBHjuJxkL1y/)

<a href="/cs/sprint/recndTnO1V8oDbBPb" class="bd-toc-link">7.  Computer Architecture</a>

- [Computer Architecture: Basics, Number Bases](/cs/module/recsuJbrrFgbFUCRX/)
- [Computer Architecture: Bitwise Operations](/cs/module/rec2NHr4Eyib7XdED/)
- [Computer Architecture: The System Stack](/cs/module/recvQUkzz23NTj20G/)
- [Computer Architecture: Subroutines, CALL/RET](/cs/module/recGPVAdvQcmopSIO/)

<a href="/cs/sprint/reco0t22NdXmr8VyL" class="bd-toc-link">8.  CS Unit 2 Build</a>

---

<a href="/" class="navbar-brand"><img src="data:image/svg+xml;base64," /></a>

#### Computer Science Legacy

<a href="/cs/sprint/recd4D4w3QrigPqUF" class="bd-toc-link">1.  Intro to Python and OOP</a>

- [Introduction to Python I](/cs/module/recay2erzDlYUPSeO/)
- [Introduction to Python II](/cs/module/recwpe3Y9TVWrGT8L/)
- [Introduction to Python III](/cs/module/reca7NYptklr7F403/)
- [Introduction to Python IV](/cs/module/recc3eWphKVYd0oHT/)

<a href="/cs/sprint/recR4gHcvD21ziR9a" class="bd-toc-link">2.  Data Structures</a>

- [Data Structures I](/cs/module/rec3MaMAY78iDm7ax/)
- [Data Structures II](/cs/module/recMcvOrFw5BWUku3/)
- [Data Structures III](/cs/module/recx53S3pYfDfvFDm/)
- [Data Structures IV](/cs/module/recHdwPne4Xt3A7lk/)

<a href="/cs/sprint/recd9grrKlURJ453N" class="bd-toc-link">3.  Algorithms</a>

- [Iterative Sorting](/cs/module/reck76SPX26beGSqE/)
- [Recursive Sorting](/cs/module/reccRh9h6ccXghfA4/)
- [A First-Pass Solution](/cs/module/recrCuZQMVI6LvxhD/)
- [Writing Better Solutions](/cs/module/recsvJCzPlM2X63ZX/)

<a href="/cs/sprint/recvDjRQEq49uoWsU" class="bd-toc-link">4.  CS Unit 1 Build</a>

<a href="/cs/sprint/recAr3gdL8U57eho1" class="bd-toc-link">5.  Hash Tables</a>

- [Hash Tables I](/cs/module/recSwIvbSV630gdVk/)
- [Hash Tables II](/cs/module/recHzCwboKBLBB0Re/)
- [Hash Tables III & IV](/cs/module/recsEDFseukQWg92c/)

<a href="/cs/sprint/rec7U9K7OCL5ihj0t" class="bd-toc-link">6.  Graphs</a>

- [Graphs I](/cs/module/recBMbHtb8AOXq3UL/)
- [Graphs II](/cs/module/recZL2m6Gx7B4dU3G/)
- [Graphs III](/cs/module/reck4RVWsg82eiYPZ/)
- [Graphs IV](/cs/module/recoGWlBHjuJxkL1y/)

<a href="/cs/sprint/recndTnO1V8oDbBPb" class="bd-toc-link">7.  Computer Architecture</a>

- [Computer Architecture: Basics, Number Bases](/cs/module/recsuJbrrFgbFUCRX/)
- [Computer Architecture: Bitwise Operations](/cs/module/rec2NHr4Eyib7XdED/)
- [Computer Architecture: The System Stack](/cs/module/recvQUkzz23NTj20G/)
- [Computer Architecture: Subroutines, CALL/RET](/cs/module/recGPVAdvQcmopSIO/)

<a href="/cs/sprint/reco0t22NdXmr8VyL" class="bd-toc-link">8.  CS Unit 2 Build</a>

---

- [Prepare](#prepare)
- [Learn](#learn)
- [Project](#project)
- [Review](#review)

# Data Structures II

<span class="lead"> </span>

In this module, we will learn the reasons that justify using a linked list as the underlying data structure for a queue or stack. We will also learn about doubly linked lists and how to construct a Doubly Linked List class in Python.

**At the end of this module, you should be able to:**

- justify using a linked list for queues and stacks instead of a Python list
- construct a doubly linked list

#### Pro Tip

“AI” solves the problem you give it - a system trained to play Tetris learned it could avoid losing by pausing.

## <a href="#prepare" id="prepare" class="anchor"><span class="octicon octicon-link"></span></a>Prepare

Review each preclass resource before class.

- # An error occurred.

  [Try watching this video on www.youtube.com](https://www.youtube.com/watch?v=j-fBJpSQ19g), or enable JavaScript if it is disabled in your browser.

## <a href="#learn" id="learn" class="anchor"><span class="octicon octicon-link"></span></a>Learn

#### Learn to justify using a linked list for queues and stacks instead of a Python list

##### Overview

We can envision our computer’s memory as a giant array, where each slot in the array has its own index (or address) for quick access. When we initialize an array in our code, the contents of that array are laid out contiguously in memory. In other words, you can think of those array elements as existing right next to each other physically in your computer’s memory.

This property of arrays is one of the reasons why we’re able to access their elements efficiently; we’re partly taking advantage of the physical locality of those elements, since they’re so tightly packed together.

Linked lists, in contrast, lack this contiguous property; their elements are not physically located right next to each other in memory. Individual linked list elements are strewn across your computer’s memory. In the context of a singly-linked list, each element only knows about itself and the next linked list node in the list. More specifically, each linked list element knows the memory address of the next element in the list.

##### Follow Along

Given some background on linked lists and arrays, what are some tradeoffs that result from the differences between both data structures?

With arrays, their contiguous nature must always be upheld. This means that whenever we want to add or remove an element from an array, other elements have to get shifted around in order to accommodate this. In the worst case (adding/removing an element at the front of the array), this results in every other element having to be moved, which can be expensive, especially if there are a lot of elements in the array.

With linked lists, adding/removing elements is technically cheaper in comparison. All that needs to be done is the new element needs to be set to point to its next element, and the element that goes before the new element needs to be set to point to it. In essence, changes to linked lists only affect a few local nodes, not the whole list itself.

##### Challenge

1.  Model/draw out the process detailed above. What are the steps involved in removing an element from the middle of an array? What are the steps involved in removing an element from the middle of a linked list?
2.  While we mentioned that changes to linked lists only affect a few local nodes in the list, is removing an element from the middle of a linked list actually cheaper than removing an element from the middle of an array? Why or why not?

---

#### Learn to construct a doubly linked list

##### Overview

### Drawbacks of a singly linked list

- Unidirectional traversal meant that removing a node from the end of a singly linked list is a linear operation instead of a constant-time operation.
- Getting to any position in a singly-linked list requires traversing from the head of the list, even if the position we want to get to is physically closer to the tail.

##### Follow Along

Model/draw out the processes of inserting and removing nodes from a doubly linked list.

##### Challenge

Complete the doubly linked list implementation.

---

## <a href="#project" id="project" class="anchor"><span class="octicon octicon-link"></span></a>Project

- ##### [Data Structures](https://github.com/LambdaSchool/data-structures)

  A repository where students explore and implement a number of data structures, namely linked lists, queues, binary search trees, and heaps.

## <a href="#review" id="review" class="anchor"><span class="octicon octicon-link"></span></a>Review

### Class Recordings

You can use class recordings to help you master the material.

- **[Data Structures II for CSPT12 w/ Elissa Thomas](https://youtu.be/97Hp2uc0bGA)**

  •Review removing singly liked list tail node. •Justify using a singly linked list for queues and stacks instead of a Python list. •Construct a doubly linked list.

- [All previous recordings](/archive/CS/module/recMcvOrFw5BWUku3)

### Demonstrate Mastery

To demonstrate mastery of this module, you need to complete and pass a code review on each of the following:

- Objective challenge:

  1.  Model/draw out the process detailed above. What are the steps involved in removing an element from the middle of an array? What are the steps involved in removing an element from the middle of a linked list?
  2.  While we mentioned that changes to linked lists only affect a few local nodes in the list, is removing an element from the middle of a linked list actually cheaper than removing an element from the middle of an array? Why or why not?

- Objective challenge:

  Complete the doubly linked list implementation.

- Project: Data Structures
